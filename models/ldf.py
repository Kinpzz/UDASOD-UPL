...
def weight_init(module):
    for n, m in module.named_children(): # 只返回第一层子模块。
        # print('initialize: '+n)
...
class Bottleneck(nn.Module):
    """
    inplanes: input channel for this block 
    planes: bottleneck channel
    planes * block.epansion: output channel for this 
    """
...
class ResNet(nn.Module):
    ...
    def make_layer(self, planes, blocks, stride, dilation): 
        # for this layer:self.inplanes is the output channel of last layer
        # planes * block.epansion is input channels of blocks of this layer
    ...
    def initialize(self, backbone_path = None):
        if backbone_path:
            print(f"resnet load model from {backbone_path}")
            self.load_state_dict(torch.load(backbone_path), strict=False)
        else:
            print(f"resnet random init")
            weight_init(self)
